...
render_scad_file() {
    ...
    if [[ $x -eq 0 && $y -eq 0 ]]; then
        ...
        cura_file="${output_dir}/${battery_type}.cura_fixed.stl"
        ...
        cura_file="${output_dir}/${battery_type}.${x}x${y}.cura_fixed.3mf"
    ...
    if [ $? -eq 0 ]; then
        echo "Completed SCAD Render to $openscad_file, starting Cura"
        ...
        cura-engine slice -i "$openscad_file" -o "$cura_file"
        if [ $? -eq 0 ]; then
            echo "Completed cura fix to $cura_file, all done"

            # Clean up and rename
            mv "$cura_file" "$final_file"
            rm -f "$openscad_file"

            final_file_size=$(stat --format="%s" "$final_file")
            echo "Done with cura entirely. The size of the file $final_file is $final_file_size bytes."

            # Calculate the difference
            file_size_difference=$((final_file_size - file_size))

            # Echo the difference
            echo "Cura saved us $file_size_difference bytes."
        else
            echo "Cura failed, keeping OpenSCAD output"
